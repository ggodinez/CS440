...
Gail Godinez
Collaborative Partner: Kevin Amorim
...
class WumpusWorldAgent(logic_440.KnowledgeBasedAgent):
  def __init__(self, cave_size):
    ...
    ##
    for q in range(1, cave_size+1):
      for p in range(1, cave_size+1):
        senb = "B%d_%d" % (q,p)
        sens = "S%d_%d" % (q,p)
        senw = ""
        senp = ""
        ##
        for (x, y) in get_neighbors(q, p, cave_size):
          senp = senp + ("P%d_%d" % (x,y))+ "|"
          senw = senw + ("W%d_%d" % (x,y))+ "|"
        ##  
        senp = senp[:len(senp)-1]
        senw = senw[:len(senw)-1]
        
        self.KB.tell(logic.Expr("<=>", sens, senw))
        self.KB.tell(logic.Expr("<=>", senb, senp))
  ...
  def safe(self):
    ...
    # FIXME - use logic to determine the set of locations I can prove to be
    # safe
  ...
  def not_unsafe(self):
    ...
    # FIXME - use logic to determine the set of locations I can't prove to be
    # unsafe
  ...
  def unvisited(self):
    ...
    # FIXME - use logic to determine the set of locations I haven't visited
    # yet.
...
def main():
  ...
  play(World(5, (3, 2), ((5, 1), (2, 4), (5, 4), (3,3)), (3, 4)))
